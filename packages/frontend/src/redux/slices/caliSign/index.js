...
import { selectAccountId, selectAccountUrlShard, selectAccountUrlToken } from '../account';
    ...
    async (_, thunkAPI) => {
        ...
        await Mixpanel.withTracking(mixpanelName,
            async () => {
                ...
                try {
                    const shardId =  selectAccountUrlShard(getState());
                    const token =  selectAccountUrlToken(getState());
                    const shardConfig = {
                        networkId: 'default',
                        nodeUrl: `https://api.testnet.calimero.network/api/v1/shards/${shardId}/neard-rpc`,
                        token,
                    };
                    //"5bc63073b1ad22bb598b11300de01642d865cfe151a000ee3adff8c7d1fa9178"
                    const args = { url: shardConfig.nodeUrl + '/' ,
                        headers: {
                            'x-api-key' : shardConfig.token,
                        }
                    };

                    const wallet = new Wallet();
                    wallet.connection = nearApiJs.Connection.fromConfig({
                        networkId: 'default',
